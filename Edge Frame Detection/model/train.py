...
def train(k,aug, model,train_x,train_y,test_x,test_y,train_SW,test_SW):
    log.info("Start training \n")
    initial_learning_rate = 0.001#exponentially decay learning rate
    lr_schedule = tf.keras.optimizers.schedules.ExponentialDecay(
                  initial_learning_rate,decay_steps=80,decay_rate=0.95,staircase=True)#exponential_decay：decayed_learning_rate = learning_rate * decay_rate ^ (global_step / decay_steps)
                  #400 0.98
                  #80 0.95
    ...
    _history = model.fit_generator(aug.flow(train_x,train_y,sample_weight=train_SW,batch_size=batch_size,seed=1),
                        validation_data=(test_x,test_y,test_SW),steps_per_epoch=len(train_x)//batch_size,
...
if __name__ =="__main__":
    ...
    for train_index,test_index in kf.split(images_X, images_Y.argmax(1)):
        ...
        for idx in tr:
            tmp_X,tmp_Y,tmp_SW = data_input.load_data("../data/cv/" + images_X[idx], \
                                        norm_size, class_num)
            if(len(tmp_X) != 0):
                train_X.append(tmp_X)
                train_Y.append(tmp_Y)
                train_SW.append(tmp_SW)
        train_X = np.concatenate(train_X); train_Y = np.concatenate(train_Y); train_SW = np.concatenate(train_SW)
        ...
        for idx in te:
            ...
            if(len(tmp_X) != 0):
                test_X.append(tmp_X)
                test_Y.append(tmp_Y)
                test_SW.append(tmp_SW) 
        test_X = np.concatenate(test_X); test_Y = np.concatenate(test_Y); test_SW = np.concatenate(test_SW)
        ...
        for ovrs in range(distance):
            train_X = np.append(train_X, train_X[idx_of_caries[0][oversampling],:,:].reshape(1,64,64,1),axis=0)
            train_Y = np.append(train_Y,train_Y[idx_of_caries[0][oversampling],:].reshape(1,2),axis=0)
            train_SW = np.append(train_SW,train_SW[idx_of_caries[0][oversampling]])
        ...
        for ovrs in range(distance):
            test_X = np.append(test_X, test_X[idx_of_caries[0][oversampling],:,:].reshape(1,64,64,1),axis=0)
            test_Y = np.append(test_Y,test_Y[idx_of_caries[0][oversampling],:].reshape(1,2),axis=0)
            test_SW = np.append(test_SW,test_SW[idx_of_caries[0][oversampling]])
        ...
        hist.append(train(k,aug,model,train_X,train_Y,test_X,test_Y,train_SW,test_SW))#训练
