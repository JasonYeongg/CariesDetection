...
def crop_image(img,oimg,side,caries,samplew,croplists,bordersize,idx,outdir,predict=0):
...
def crop_image_conly(img,oimg,side,caries,samplew,croplists,bordersize,idx,outdir,predict=0):
    log.info("running function \"crop_image\" \n")
    #fault會使得我們注意框的中間的caries數量，如果太少就認爲這個框是normal
    fault = int(bordersize/4)#accept how many caries in frame
    fcheck = int(bordersize*0.8)#accept how many houghline in frame
...
if __name__ == '__main__': #防止調用的時候這一行被運行
    ...
    #for label in ["normal", "caries"]:
    for label in [ "caries","normal"]:
        #原圖像
        image = glob.glob(r"../mask/" + label + r"/teeth/*")
        if (len(image) == 0):
            log.warning("%s was empty...\n" % (r"../mask/" + label + r"/teeth/*"))
        for idx,element in enumerate(image):
            ...
            #牙齒形狀圖，讀進來會有兩個元素，第一個元素的最後一維表示牙齒數量
            #每一個都是一個灰階圖，代表圖片中某顆牙齒，可用canny求edges
            #第二個元素是一個list，其中list[0]=1代表第一顆牙是雙牙根，0為單牙根
            teeth_mask   = r"../mask/" + label + r"/teeth_mask/" + filename + ".npy"
            #caries_mask   = glob.glob(r"../mask/caries_mask/") #可以用組織分類就好
            #組織分類圖，其中不同組織用不同數值表示,5是牙齦，6是蛀牙
            tissue_map   = r"../mask/" + label + r"/tissue_map/" + filename + ".npy"
            caries_mask = r"../mask/" + label + r"/caries_mask/" + filename + ".txt"
            
            #讀取圖片
            oimg = cv2.imread(image[idx])
            img = oimg.copy()
            overlay = oimg.copy()
            overlayo = oimg.copy()
            py, px = oimg.shape[0], oimg.shape[1]
            #讀取teeth mask
            with open(teeth_mask, 'rb') as m:
                t_mask = np.load(m, allow_pickle=True)
            #讀取tissue map
            with open(tissue_map, 'rb') as t:
                ts_map = np.load(t, allow_pickle=True)
            #讀取caries mask -----------e1e2 got maybe 10% teeth width
            with open(caries_mask, 'rb') as f: 
                caries = np.loadtxt(f)
            #把牙齦分割出來並放大gums的部分
            ...
            #kernel = np.ones((int(py/25),int(py/25)),np.uint8)
            kernel = np.ones((int(py/10),int(py/10)),np.uint8)
            gums = cv2.dilate(gums,kernel,iterations = 1)
            
            caries = convert_bool_uint8(caries) #將caries值map到0和255並轉成uint8
			
            t_mask = convert_bool_uint8(t_mask[0]) #將t_mask值map到0和255並轉成uint8
            
            if len(t_mask.shape) == 2: #代表圖片中沒有牙齒(第48張)
                log.warning("No teeth in %s \n" % image[idx])
                continue
            ...
            img = only_teeth(img,t_mask,px,py,twidth)#除了teeth以外的部分都變成黑色
            
            edges = get_edges(t_mask,twidth) #利用縮小後的edge取框使得框在裏面一點
			
            bordersize, croplists = get_croplists(gums,twidth,edges,px,py) #獲取crop的位置(y/row,x/col,y/row+h,x/col+w)
            
            ''' #這部分老師說不必做，因爲test的時候我們無法用同樣的方式得到
            if np.count_nonzero(caries) != 0:
                caries_crop = crop_by_caries(caries,bordersize,px,py) #按照caries位置獲取框
                croplists.append(caries_crop)
            '''
			
            side,hline = find_side(edges,oimg)
			
            ########augmentation by ourself-------------------------------------#######
            twidth1 = twidth*1.3
            bordersize1, croplists1 = get_croplists(gums,twidth1,edges,px,py,0.2,15) #獲取crop的位置
            twidth2 = twidth*0.7
            bordersize2, croplists2 = get_croplists(gums,twidth2,edges,px,py,0.1,30) #獲取crop的位置
			
            twidth3 = twidth*0.85
            bordersize3, croplists3 = get_croplists(gums,twidth3,hline,px,py,0.1,30) #獲取crop的位置		
			
            twidth4 = twidth*1.1
            bordersize4, croplists4 = get_croplists(gums,twidth4,hline,px,py,0.1,30) #獲取crop的位置		
            twidth5 = twidth*0.6
            bordersize5, croplists5 = get_croplists(gums,twidth5,hline,px,py,0.1,30) #獲取crop的位置	
            twidth6 = twidth*0.9
            bordersize6, croplists6 = get_croplists(gums,twidth6,hline,px,py,0.1,30) #獲取crop的位置	
            twidth7 = twidth*0.95
            bordersize7, croplists7 = get_croplists(gums,twidth4,hline,px,py,0.1,30) #獲取crop的位置		
            twidth8 = twidth*0.65
            bordersize8, croplists8 = get_croplists(gums,twidth5,hline,px,py,0.1,30) #獲取crop的位置	
            twidth9 = twidth*0.8
            bordersize9, croplists9 = get_croplists(gums,twidth6,hline,px,py,0.1,30) #獲取crop的位置	
            ######--------------------------------------------------------------#######
            ...
            #把框切出來
            oimg,samplew,predict_list = crop_image(img,oimg,side,caries,samplew,croplists,bordersize,idx,outdir)
            swmap = oimg.copy()

            
            ########augmentation by ourself-------------------------------------#######
            oimg,samplew,predict_list = crop_image(img,oimg,side,caries,samplew,croplists1,bordersize1,idx,outdir)
			
            oimg,samplew,predict_list = crop_image(img,oimg,side,caries,samplew,croplists2,bordersize2,idx,outdir)
			
            #oimg,samplew,predict_list = crop_image(img,oimg,side,caries,samplew,croplists3,bordersize3,idx,outdir)

            oimg,samplew,predict_list = crop_image_conly(img,oimg,side,caries,samplew,croplists4,bordersize4,idx,outdir)
            oimg,samplew,predict_list = crop_image_conly(img,oimg,side,caries,samplew,croplists5,bordersize5,idx,outdir)
            oimg,samplew,predict_list = crop_image_conly(img,oimg,side,caries,samplew,croplists6,bordersize6,idx,outdir)
            oimg,samplew,predict_list = crop_image_conly(img,oimg,side,caries,samplew,croplists7,bordersize7,idx,outdir)
            oimg,samplew,predict_list = crop_image_conly(img,oimg,side,caries,samplew,croplists8,bordersize8,idx,outdir)
            oimg,samplew,predict_list = crop_image_conly(img,oimg,side,caries,samplew,croplists9,bordersize9,idx,outdir)
            ######--------------------------------------------------------------#######
