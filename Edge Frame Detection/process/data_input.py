...
def load_cvdata(cv_path):
    ...
    for each_path in t:
        ...
        if (len(normal) > 0): 
            if (len(caries) <= 0) and (random.random() > 0.6): 
...
def load_txt(each_path):
    #log.info("running function \"load_swtxt\" \n")

    filepath, full_filename = os.path.split(each_path)
    filepath,labelll = os.path.split(filepath)
    filepath,dataname = os.path.split(filepath)
    filename, suffix = os.path.splitext(full_filename)
    #.../data/cv(filepath)    c(filename)    pic(dataname)    label(labelll)
    
    cpd = pd.read_csv(filepath  + r"/" + dataname + r"/sw/c.txt",sep='\t')#轉換文字到array
    c = np.array(cpd)
    
    num = np.where(c == filename)#filename search c array的位置
    num = num[0].astype(int)
    
    swpd = pd.read_csv(filepath  + r"/" + dataname + r"/sw/sw.txt",sep='\t')#轉換文字到array
    sw = np.array(swpd)
    ssw = sw[num]#用位置在sw array找到sw
...
def load_data(path, norm_size, class_num, predict = 0):
    ...
    for each_path in image_paths:
        ...
        if maker == 0 and random.random() > 0.7:
            label.append(maker)
            data.append(image)
            sw.append(ssw)
        elif maker == 1:
