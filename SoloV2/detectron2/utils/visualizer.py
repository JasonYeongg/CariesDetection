...
def split_mask_n_duplicate_array(masks,scores,classes):
			
    dmasks = []
    dscores = []
    dclasses = []
	
    for mask,score,classe in zip(masks,scores,classes):
    ...
    return dmasks, dscores, dclasses

def extract_bboxes(masks):
    bboxes = []
    for mask in masks:
        m = GenericMask(mask, mask.shape[0], mask.shape[1])
...
def oridatahandle(dataname):

    with open('caries/datasett.txt', 'r') as f:
        vdata = np.loadtxt(f)
    ...
    for vlist in vdata:
        frame = []
        if int(vlist[0]) == int(dataname):
            check = 1
            frame.append((vlist[2],vlist[3],vlist[4],vlist[5]))
            data.append((frame,int(vlist[1])))
        elif check==1 and int(vlist[0]) != int(dataname):
            return data
    return data
		
def box_intersection_over_union(frame1, frame2):
...
def intersection_over_union(frame1, frame2):
    # compute the area of intersection rectangle
    interArea = np.count_nonzero(np.logical_and(frame1 ,frame2))
    # compute the area of both the prediction and ground-truth
    # rectangles
...
class Visualizer:
    ...
    def draw_instance_predictions(self, predictions):
		...
		#------------------------------------------------------------------------------------------------------------------	
        masks,scores,classes = split_mask_n_duplicate_array(masks,scores,classes)
        boxes = extract_bboxes(masks)
        sortdata =  sorted(zip(scores,classes,boxes,masks), reverse=True  , key = lambda x: (x[1], x[0]))
        ...
        while (sortlist): #NMS
            ...
            for num in range(1,len(sortlist)):
                iou = intersection_over_union(sortlist[0][3], sortlist[num][3])
                if iou > 0.05:
        ...
        for datalist in oridatahandle(dataname):
            #print(datalist[1], list[1],list[2])
            for list in selectdata:
                iou = box_intersection_over_union(np.array(datalist[0][0]), list[2])
                if iou > 0.85:
                    if int(datalist[1]) == int(list[1]): 
                        if int(datalist[1]) == 1:
                            TN += 1
                        else:
                            TP += 1
